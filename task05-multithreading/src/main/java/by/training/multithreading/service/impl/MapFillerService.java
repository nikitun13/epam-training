...
import by.training.multithreading.service.thread.MapFillerThread;
...
public class MapFillerService implements ParallelMatrixFillerService {

    private static final Logger log
            = LogManager.getLogger(MapFillerService.class);
    ...
    public void fillMatrix(Matrix matrix, String pathToThreadsConfig)
        ...
        try {
            ...
            ConcurrentMap<Integer, Integer> map = new ConcurrentHashMap<>(matrix.getNumberOfRows(), 1f);
            List<MapFillerThread> threads = threadsConfig.stream()
                    .map(value -> new MapFillerThread(value, matrix, map))
                    .toList();
