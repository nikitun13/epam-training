...
import by.training.multithreading.service.thread.AtomicFillerThread;
...
/**
 * The class {@code AtomicFillerService}
 * is a class that implements {@link ParallelMatrixFillerService}.<br/>
 * Uses {@link AtomicInteger} to fill Matrix's main diagonal.
 *
 * @author Nikita Romanov
 */
public class AtomicFillerService implements ParallelMatrixFillerService {

    private static final Logger log
            = LogManager.getLogger(AtomicFillerService.class);
    ...
    public void fillMatrix(Matrix matrix, String pathToThreadsConfig)
        ...
        try {
            ...
            List<AtomicFillerThread> threads = threadsConfig.stream()
                    .map(value -> new AtomicFillerThread(value, matrix, atomicInteger))
                    .toList();
