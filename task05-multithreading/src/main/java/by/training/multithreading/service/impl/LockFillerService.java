...
import by.training.multithreading.service.thread.LockFillerThread;
 ...
 * Uses {@link ReentrantLock} to fill Matrix's main diagonal.
 *
 * @author Nikita Romanov
 */
public class LockFillerService
        implements ParallelMatrixFillerService {

    private static final Logger log
            = LogManager.getLogger(LockFillerService.class);
    ...
    public void fillMatrix(Matrix matrix, String pathToThreadsConfig)
        ...
        try {
            ...
            List<LockFillerThread> threads = threadsConfig.stream()
                    .map(value -> new LockFillerThread(value, matrix, lock))
                    .toList();
